...
export default function FujianClassicRouteCompact({ props }: { props: LocalizedTripProps }) {
  ...
  return <main className={`trip-detail-page ${styles.themePage} ${styles.compactPage} ${styles.expV3Page}`} lang={props.locale === "zh-TW" ? "zh-Hant" : props.locale}>
    ...
    <section className={styles.akOverview}><div className="shell"><div className={styles.akOverviewIntro}><p className={styles.classicEyebrow}>THE JOURNEY</p><h2>第一次来福建，这五天刚刚好。</h2><p>五天时间，从厦门和鼓浪屿开始，走进福建土楼的山谷，再到泉州认识真正的闽南。这是一条适合第一次来福建的路线，不追求一天打卡很多景点，而是在有限的时间里，看见几个真正不同的福建。</p></div><div className={styles.akOverviewMeta}><p>5 DAYS</p><strong>厦门 → 鼓浪屿<br />→ 福建土楼 → 泉州</strong><span>PRIVATE JOURNEY</span><span>FLEXIBLE</span><a href={plan}>开始定制这条路线 →</a></div></div></section>
